...
class LoggingTest < ActiveSupport::TestCase
  describe RocketJob::Subscribers::Logger do
    ...
    it 'Changes specific log level from debug to trace' do
      RocketJob::Subscribers::Logger.new.level(class_name: 'RocketJob::Supervisor', level: :trace)
    ...
    it 'Changes global log level from debug to trace' do
      ...
      RocketJob::Subscribers::Logger.new.level(level: :trace)
    ...
    it 'Filters based on host_name' do
      ...
      RocketJob::Subscribers::Logger.new.level(class_name: 'RocketJob::Supervisor', level: :trace, host_name: host)
      assert_equal :trace, RocketJob::Supervisor.logger.level
    end
    ...
    it 'Filters based on pid' do
      process_id = $$
      RocketJob::Subscribers::Logger.new.level(class_name: 'RocketJob::Supervisor', level: :trace, pid: process_id)
      assert_equal :trace, RocketJob::Supervisor.logger.level
    end
